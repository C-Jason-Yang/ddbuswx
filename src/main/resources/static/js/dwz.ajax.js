...
/**
 * 重置navTab上的查询, 会重新载入当前navTab
 * @param {Object} form
 */
function resetNavTabSearch(form, navTabId){
    var $form = $(form);
    if (form[DWZ.pageInfo.pageNum])
    	form[DWZ.pageInfo.pageNum].value = 1;
    var tempFormData = $form.serializeArray();
    tempFormData.push({"isReset": true});
    navTab.reload($form.attr('action'), {data: tempFormData, navTabId:navTabId});
    return false;
}
...
function dwzPageBreak(options){
    ...
    if(op.targetType == "jbsxBox")
    {
        var $box = $("div").find("#" + op.rel.split(";")[0]);
        var form = _getPagerForm($box, op.data);
        if (form)
        {
            $box.ajaxUrl({
                type:"POST", url:op.rel.split(";")[1], data: $(form).serializeArray(), callback:function(){
                    $box.find("[layoutH]").layoutH();
                }
            });
        }
        // var $box = $("#" + op.rel);
        // var form = _getJbsxBoxPagerForm(op.rel, op.data);
        // if (form) {
        // 	$box.ajaxUrl({
        // 		type:"POST", url:$(form).attr("action"), data: $(form).serializeArray(), callback:function(){
        // 			$box.find("[layoutH]").layoutH();
